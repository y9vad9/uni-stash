...
public class MultiplyMatrixToVector {
    public static void main(String[] args)
        ...
        // matrix size (ord х ord)
        int ord = 16;
        // the number of rows for each processor
        int k = ord / size;
        // the last portion will be n (less than k) elements )
        int n = ord - k * (size - 1);
        if (rank == 0) {
            ...
            Element[][] M0 = new Element[n][];
            int[][] col0 = new int[n][];
            for (int s = 0; s < n; s++) {
                M0[s] = A.M[s + (size - 1) * k];
                col0[s] = A.col[s + (size - 1) * k];
            }
            MatrixS A0 = new MatrixS(M0, col0);
            VectorS V0 = A0.multiplyByColumn(B.
                transpose(ring), ring);
            System.arraycopy(V0.V, 0, result,
                (size - 1) * k, n);
            for (int t = 1; t < size; t++) {
                VectorS Vt = (VectorS) Transport.
                    recvObject(t, t);
                System.arraycopy(Vt.V, 0, result,
                    (t - 1) * k, k);
            }
