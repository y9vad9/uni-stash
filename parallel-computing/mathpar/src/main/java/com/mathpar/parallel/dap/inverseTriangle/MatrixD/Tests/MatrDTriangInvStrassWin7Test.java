...
public class MatrDTriangInvStrassWin7Test extends DAPTest {
    private final static MpiLogger LOGGER = MpiLogger.getLogger(MatrDTriangInvStrassWin7Test.class);

    protected MatrDTriangInvStrassWin7Test() {
        super("MatrDTriangInvStrassWin7", 18, 0);
    ...
    public static void main(String[] args) throws InterruptedException, MPIException {
        ...
        new MatrDTriangInvStrassWin7Test().runTests(args);
    }
...
*/
    ...
    protected Element[] sequentialExecute(Element[] data, Ring ring) {
        return new Element[] {((MatrixD) data[0]).inverseLowTriangleWin(ring)};
    }
}
