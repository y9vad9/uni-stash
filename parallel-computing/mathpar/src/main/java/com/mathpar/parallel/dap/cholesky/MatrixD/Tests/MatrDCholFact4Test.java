...
import com.mathpar.parallel.dap.cholesky.MatrixS.Tests.MatrSCholFact4Test;
...
public class MatrDCholFact4Test extends DAPTest {
    private final static MpiLogger LOGGER = MpiLogger.getLogger(MatrDCholFact4Test.class);
    MatrixD initMatrix;

    protected MatrDCholFact4Test() {
        super("MatrDCholFact4", 21, 0);
        // ring = new Ring("R[]");
...
//        MatrixS check = (MatrixS) resultCheck[0];
    ...
    public static void main(String[] args) throws InterruptedException, MPIException {
        MatrDCholFact4Test test = new MatrDCholFact4Test();
        test.runTests(args);
    }
    ...
    protected Element[] sequentialExecute(Element[] data, Ring ring) {
        return ((MatrixD) data[0]).choleskyFactorize4(ring);
    }
}
