...
public class MutableBigInteger {
 ...
 public   MutableBigInteger() {
  ...
  public  MutableBigInteger(int val) {
     ...
     */
 public  MutableBigInteger(int[] val, int len) {
        value = val;
        intLen = len;
    }
   ...
   public  MutableBigInteger(int[] val) {
     ...
     */
public    MutableBigInteger(NumberZ b) {
        value = b.mag.clone();
        intLen = value.length;
    }
  ...
  public  MutableBigInteger(MutableBigInteger val) {
    ...
    private final int getLowestSetBit() {
        ...
        if (b==0)
            return -1;
        return ((intLen-1-j)<<5) + NumberZ.trailingZeroCnt(b);
    }
    ...
    void rightShift(int n) {
        ...
        if (nBits == 0)
            return;
        int bitsInHighWord = NumberZ.bitLen(value[offset]);
    ...
    void divideOneWord(int divisor, MutableBigInteger quotient) {
        ...
        int shift = 32 - NumberZ.bitLen(divisor);
     ...
     *
     */
  public  void divide(MutableBigInteger b,
        ...
        int shift = 32 - NumberZ.bitLen(d[0]);
        if (shift > 0) {
            // First shift will not grow array
            NumberZ.primitiveLeftShift(d, dlen, shift);
            // But this one might
            rem.leftShift(shift);
        }
    ...
    private boolean unsignedLongCompare(long one, long two) {
        return (one + NumberZ64.MIN_VALUE) > (two+NumberZ64.MIN_VALUE);
    }
  ...
  public  MutableBigInteger hybridGCD(MutableBigInteger b) {
    ...
    static int binaryGcd(int a, int b) {
        ...
        int y = NumberZ.trailingZeroTable[x];
        aZeros += y;
        a >>>= y;
        ...
        while (a != b) {
            if ((a+0x80000000) > (b+0x80000000)) {  // a > b as unsigned
                ...
                while ((x = a & 0xff) == 0)
                    a >>>= 8;
                a >>>= NumberZ.trailingZeroTable[x];
            } else {
                b -= a;

                while ((x = b & 0xff) == 0)
                    b >>>= 8;
                b >>>= NumberZ.trailingZeroTable[x];
            }
        }
        return a<<t;
    }
  ...
  public static int inverseMod32(int val) {
     ...
     */
    public MutableBigInteger modInverse(MutableBigInteger mod) {
    ...
    void setBorders(int a, int b) {
      if ((a>=0) && (b<this.value.length) && (a <= b)) {
        this.intLen = b-a+1;
        this.offset = a;
      }
      else {
        throw new ExceptionInInitializerError("WrongBordersException");
      }
    }
